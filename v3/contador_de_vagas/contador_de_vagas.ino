...
const char* ssid = "Rep5-As"; //Your internet SSID here
...
void loop() {
  ...
  if (now - lastMsg > 1000) {
    ...
    client.publish((user+"/numvagas").c_str(), String(numVagas).c_str());
    client.publish((user+"/RX").c_str(), String(numVagas).c_str());
    ...
    if (d3 > 20 && numVagas == 0) {
      delay(300);
      if (!travar) {
        servoMotor.write(8); //cancela fecha
        travar = true;
      }
    }
    else if (!travar)
      servoMotor.write(180); //cancela abre
  }
